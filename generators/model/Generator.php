        ...
        foreach ($table->columns as $column) {
            ...
            switch ($column->type) {
                ...
                default: // strings
                    ...
                    $types['app\validators\DelspacesValidator'][] = $column->name;
                    $filters['strip_tags'][] = $column->name;
                    //$filters['addslashes'][] = $column->name;
            }
        }
        $rules = [];
        foreach ($filters as $filter => $columns) {
            $rules[] = "[['" . implode("', '", $columns) . "'], 'filter', 'filter' => '$filter']";
        }
        foreach ($types as $type => $columns) {
            if ($type == 'date') {
                $rules[] = "[['" . implode("', '", $columns) . "'], '$type', 'format' => 'yyyy-M-d H:m:s']";
            } else {
