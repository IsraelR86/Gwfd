...
<DIV STYLE="background-color: #EE5656; font-family: 'Century Gothic';">
	...
	<DIV STYLE="background-color: #FFFFFF; border-radius:10px; margin: 20px; padding-right: 30px; padding-left: 30px; min-width: 510px">
		...
		<BR>
		<P STYLE="color: #48C0C3; font-size: 40pt;">¡APLICASTE A UN CONCURSO!</P>
		<P STYLE="text-align: center; font-size: 21pt;">¡Nos encanta que apliques a nuevos concursos!</P>
		<P STYLE="font-size: 14pt;">A continuación los datos del concurso al que aplicaste:
		<BR>
		<BR>Fecha y hora: <?= $fecha ?>
		<BR>Clave del concurso: <?= $id_concurso ?>
		<BR>Nombre del Concurso: <?= $concurso ?>
		<BR><IMG SRC="<?= $message->embed($imagen_concurso) ?>" STYLE="height:250px; width:250px;">
		<BR>Proyecto con el que aplicaste: <?= $proyecto ?>		
		<BR>Folio de aplicación: <?= $folio ?>
		<BR>
		<BR>¡Te deseamos la mejor de las suertes!
		<BR>
		<BR>Un abrazo,
		<BR>
